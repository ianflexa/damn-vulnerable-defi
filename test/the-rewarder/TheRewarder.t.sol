...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        ...
        // n of txs for each claim
        uint256 dvtTxs = TOTAL_DVT_DISTRIBUTION_AMOUNT / playerDVTAmount;
        uint256 wethTxs = TOTAL_WETH_DISTRIBUTION_AMOUNT / playerWETHAmount;
        uint256 totalTxCount = dvtTxs + wethTxs;

        Claim[] memory claimDVT = new Claim[](totalTxCount);

        IERC20[] memory tokensToClaim = new IERC20[](2);
        tokensToClaim[0] = IERC20(address(dvt));
        tokensToClaim[1] = IERC20(address(weth));

        for (uint i; i < totalTxCount; i++) {
            if (i < dvtTxs) {
                claimDVT[i] = Claim({
                    batchNumber: 0, // claim corresponds to first DVT batch
                    amount: playerDVTAmount,
                    tokenIndex: 0, // DVT in `tokensToClaim`
                ...
                claimDVT[i] = Claim({
                    ...
                    tokenIndex: 1, // WETH in `tokensToClaim`
